...
class Yolo_dataset(Dataset):
    def __init__(self, label_path, cfg, train=True):
        ...
        for line in f.readlines():
            ...
            for i in data[1:]:
                print('i',i.split(';'))
                truth[data[0]].append([int(float(j)) for j in i.split(';') if not 'n/' ])

        self.truth = truth
        self.imgs = list(self.truth.keys())
        self.dataset_dir='C:/Users/WaDii/Downloads/pytorch-YOLOv4-master/TrainIJCNN2013'
    ...
    def __getitem__(self, index):
        ...
        for i in range(use_mixup + 1):
            if i != 0:
                ...
                bboxes = np.array(self.truth.get(img_path), dtype=np.float)
                img_path = os.path.join(sdataset_dir, img_path)
    ...
    def _get_val_item(self, index):
        ...
        img = cv2.imread(os.path.join(self.dataset_dir, img_path))
